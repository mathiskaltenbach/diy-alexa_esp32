...
IntentProcessor::IntentProcessor(Speaker *speaker) { m_speaker = speaker; }

IntentResult IntentProcessor::turnOnDevice(const Intent &intent) {
  const char *TAG = "IntentProcessor::turnOnDevice";
  if (intent.intent_confidence < 0.8) {
    ESP_LOGI(TAG, "Only %.f%% certain on intent\n",
             100 * intent.intent_confidence);
    return FAILED;
  }
  if (intent.device_name.empty()) {
    ESP_LOGI(TAG, "No device found");
    return FAILED;
  }
  if (intent.device_confidence < 0.8) {
    ESP_LOGI(TAG, "Only %.f%% certain on device\n",
             100 * intent.device_confidence);
    return FAILED;
  }
  if (intent.trait_value.empty()) {
    ESP_LOGI(TAG, "Can't work out the intent action");
    return FAILED;
  }
  if (intent.trait_confidence < 0.8) {
    ESP_LOGI(TAG, "Only %.f%% certain on trait\n",
  ...
  if (intent.device_name == "lights") {
    ...
    if (m_device_to_pin.find(intent.device_name) == m_device_to_pin.end()) {
      ESP_LOGI(TAG, "Don't recognise the device '%s'\n",
...
IntentResult IntentProcessor::processIntent(const Intent &intent) {
  if (intent.text.empty()) {
    ESP_LOGI(TAG, "No text recognised");
    return FAILED;
  }
  ESP_LOGI(TAG, "I heard \"%s\"\n", intent.text.c_str());
  if (intent.intent_name.empty()) {
    ESP_LOGI(TAG, "Can't work out what you want to do with the device...");
    return FAILED;
  }
  ESP_LOGI(TAG, "Intent is %s\n", intent.intent_name.c_str());

...
IndicatorLight::IndicatorLight() {
  // use the build in LED as an indicator - we'll set it up as a pwm output so
  // we can make it glow nicely
  ledcSetup(0, 10000, 8);
  ledcAttachPin(2, 0);
  ledcWrite(0, 0);
  ...
  xTaskCreate(indicatorLedTask, "Indicator LED Task", 4096, this, 1,
              &m_taskHandle);
}
...
IndicatorState IndicatorLight::getState() { return m_state; }
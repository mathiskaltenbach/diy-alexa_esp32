...
void DetectWakeWordState::enterState() {
  const char *TAG = "DetectWakeWordState::enterState";
  // Create our neural network
  m_nn = new NeuralNetwork();
  ESP_LOGI(TAG, "Created Neral Net");
  // create our audio processor
  m_audio_processor =
      new AudioProcessor(AUDIO_LENGTH, WINDOW_SIZE, STEP_SIZE, POOLING_SIZE);
  ESP_LOGI(TAG, "Created audio processor");

  m_number_of_detections = 0;
}
bool DetectWakeWordState::run() {
  const char *TAG = "DetectWakeWordState::run";
...
void DetectWakeWordState::exitState() {
  const char *TAG = "DetectWakeWordState::exitState";
  // Create our neural network
  delete m_nn;
  m_nn = NULL;

...
RecogniseCommandState::RecogniseCommandState(
    I2SSampler *sample_provider, IndicatorLight *indicator_light,
    Speaker *speaker, IntentProcessor *intent_processor) {
...
void RecogniseCommandState::enterState() {
  const char *TAG = "RecogniseCommandState::enterState";
  ...
  ESP_LOGI(TAG, "Free ram before connection %d\n", free_ram);

  m_speech_recogniser =
      new WitAiChunkedUploader(COMMAND_RECOGNITION_ACCESS_KEY);

  ESP_LOGI(TAG, "Ready for action");

  free_ram = esp_get_free_heap_size();
  ESP_LOGI(TAG, "Free ram after connection %d\n", free_ram);
}
bool RecogniseCommandState::run() {
  const char *TAG = "RecogniseCommandState::run";
  if (!m_speech_recogniser || !m_speech_recogniser->connected()) {
    ...
    ESP_LOGI(TAG, "Error - Attempt to run with no http client");
    return true;
  }
  if (m_last_audio_position == -1) {
    // set to 1 seconds in the past the allow for the really slow connection
    // time
    m_last_audio_position =
        m_sample_provider->getCurrentWritePosition() - 16000;
  }
  ...
  int sample_count = (audio_position - m_last_audio_position +
                     ...
                     m_sample_provider->getRingBufferSize();
  // ESP_LOGI(TAG,"Last sample position %d, current position %d, number samples
  ...
  if (sample_count > 0) {
    ...
    while (sample_count > 0) {
      ...
      m_speech_recogniser->sendChunkData((const uint8_t *)sample_buffer,
                                         std::min(sample_count, 500) * 2);
      sample_count -= 500;
    }
    ...
    if (m_elapsed_time > 3000) {
      ...
      ESP_LOGI(TAG, "3 seconds has elapsed - finishing recognition request");
